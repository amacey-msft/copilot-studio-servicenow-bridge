...
> The two channels are alternatives, not layers.
...
- `servicenow/` AWA queue, scripted REST. The outbound BR has a
  ...
  [`servicenow/outbound_business_rule_skill.md`](../servicenow/outbound_business_rule_skill.md))
  that POSTs to the skill's `/api/sn-webhook` so this channel gets
...
header `X-Webhook-Secret: <SN_WEBHOOK_SECRET>`. The skill silently
...
3. Say "I need to talk to a person." The orchestrator should dispatch
   to the skill. Expected reply: confirmation that a ServiceNow chat
   has been opened, with the IMS#.
...
making the user send another turn, the skill records the signed
`serviceUrl` from each inbound activity, then on every `/api/sn-webhook`
event POSTs back to
`<serviceUrl>/v3/conversations/<id>/activities` with a fresh AAD token.
CS accepts that proactive push and renders it as a new bot message.
...
`channelData.<key>`). The skill resolves it to a `sys_user.sys_id`
